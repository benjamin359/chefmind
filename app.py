...
def generate():
    try:
        ...
        prompt = f"""3-Michelin-star chef. Exceptional gourmet recipe. ALL text values in {lang_name}.
...
{{"nom":"Poetic name","sous_titre":"Evocative description","histoire":"2 inspiring sentences about origin and terroir",
...
def generate_menu():
    try:
        d = request.json or {}
        lang = d.get("lang", "en")
        prompt = f"""Michelin chef. Complete menu (starter+main+dessert). ALL text in {LANGUAGES.get(lang,"English")}.
...
Ingredients: {", ".join([i["nom"] for i in recipe.get("ingredients",[])[:6]])}
...
def get_photo():
    """Cherche une photo depuis TheMealDB — côté serveur pour éviter CORS Safari."""
    ...
    try:
        ...
        # Mots-clés à essayer dans l'ordre
        keywords = []
        if nom: keywords.append(nom.split()[0])
        for ing in ingredients[:3]:
            w = ing.get("nom","").split()[0]
            if w and w not in keywords: keywords.append(w)
        keywords += ["chicken", "pasta", "beef"]  # fallbacks garantis

        headers = {"User-Agent": "ChefMind/1.0"}

        for kw in keywords[:5]:
            try:
                url = f"https://www.themealdb.com/api/json/v1/1/search.php?s={urllib.parse.quote(kw)}"
                req = urllib.request.Request(url, headers=headers)
                with urllib.request.urlopen(req, timeout=5) as r:
                    data = json.loads(r.read())
                    meals = data.get("meals") or []
                    if meals:
                        return jsonify({"ok": True, "url": meals[0]["strMealThumb"], "source": "TheMealDB"})
            except: continue

        # Dernier recours — repas aléatoire garanti
        req2 = urllib.request.Request("https://www.themealdb.com/api/json/v1/1/random.php", headers=headers)
        with urllib.request.urlopen(req2, timeout=5) as r:
            data2 = json.loads(r.read())
            meals2 = data2.get("meals") or []
            if meals2:
                return jsonify({"ok": True, "url": meals2[0]["strMealThumb"], "source": "TheMealDB"})

        return jsonify({"ok": False, "error": "No photo found"})
    except Exception as e:
