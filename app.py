...
def ask(prompt, lang="en", big=False):
    ...
    resp = requests.post(
        ...
        json={
            "model": model,
            "messages": [
                {"role": "system", "content": f"You respond ONLY with valid JSON. No text, no backticks, no explanations. All string values in the JSON must be written in {lang_name}."},
                {"role": "user", "content": prompt}
            ],
            "temperature": 0.7,
            "max_tokens": 4096
            # Pas de response_format — cause des 400 sur certains modèles
        },
        timeout=60
    )
...
def translate_ui():
    try:
        ...
        resp = requests.post(
            ...
            json={"model": GROQ_MODEL_FAST,
                  "messages": [
                      {"role": "system", "content": "Translate UI strings. Return ONLY valid JSON with same keys. No backticks."},
                      {"role": "user", "content": prompt}
                  ],
                  "max_tokens": 4096},
            timeout=60
...
def get_photo():
    """Cherche une photo de plat depuis TheMealDB — appel serveur pour éviter CORS."""
    try:
        ...
        # Construire le mot-clé de recherche
        keyword = nom.split()[0] if nom else ""
        if not keyword and ingredients:
            keyword = ingredients[0].get("nom", "").split()[0]
        if not keyword:
            keyword = "chicken"

        # Chercher dans TheMealDB
        url = f"https://www.themealdb.com/api/json/v1/1/search.php?s={urllib.parse.quote(keyword)}"
        ...
        # Fallback — chercher "food" générique
        url2 = "https://www.themealdb.com/api/json/v1/1/random.php"
